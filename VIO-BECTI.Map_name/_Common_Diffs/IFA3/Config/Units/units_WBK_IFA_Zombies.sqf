...
if (CTI_Log_Level >= CTI_Log_Debug) then {["VIOC_DEBUG", "FILE: common\config\units\units_WBK_IFA_Zeds.sqf", format["starting units declaration: _faction: [%1]", _faction]] call CTI_CO_FNC_Log};
...
if (CTI_Log_Level >= CTI_Log_Debug) then {["VIOC_DEBUG", "FILE: common\config\units\units_WBK_IFA_Zeds.sqf", format["units declared: [%1] | [%2]", count _c, count _n]] call CTI_CO_FNC_Log};
